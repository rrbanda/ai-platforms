...
Then output a completion tag on its own line to hand control back to
the orchestrator:

```
[[COMPLETE: SDG job <job_id> finished. Model: <model>. Samples: <N>. Dataset preview shown.]]
```

If the user expressed a next intent (e.g. "Train a model on this
dataset"), append it: "User selected: Train a model on this dataset."
Do NOT instruct the orchestrator what to do — just relay the user's
choice and the factual job details.
