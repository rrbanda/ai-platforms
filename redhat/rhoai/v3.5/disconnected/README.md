...
| **Disconnected cluster** | OpenShift Container Platform **4.21+**, internal container registry (e.g. Quay, Harbor, Nexus, or mirror-registry), `cluster-admin` access |
...
| **CLI tools** | `oc` (4.21+), `oc-mirror` v2 (or `skopeo` 1.14+), `kubeseal`, `helm` (optional, for chart customization) |
...
```bash
cd redhat/rhoai/v3.5/disconnected
...
| Placeholder | Value for RHOAI 3.5 |
|---|---|
| `REPLACE_OCP_VERSION` | `v4.21` (match your cluster's minor version) |
| `REPLACE_RHOAI_CHANNEL` | `stable-3.5` |
| `REPLACE_KUEUE_CHANNEL` | `stable-v1.3` |

```bash
sed -i \
  -e 's/REPLACE_OCP_VERSION/v4.21/g' \
  -e 's/REPLACE_RHOAI_CHANNEL/stable-3.5/g' \
  -e 's/REPLACE_KUEUE_CHANNEL/stable-v1.3/g' \
...
**Note the CatalogSource name(s)** -- you will need them later (e.g. `cs-redhat-operator-index-v4-21`, `cs-certified-operator-index-v4-21`). Find them with:
...
- `REPLACE_WITH_REDHAT_CATALOG_NAME` → your Red Hat catalog name (e.g. `cs-redhat-operator-index-v4-21`)
- `REPLACE_WITH_CERTIFIED_CATALOG_NAME` → your Certified catalog name (e.g. `cs-certified-operator-index-v4-21`)
...
- Red Hat catalog: `cs-redhat-operator-index-v4-21` (from oc-mirror) or `mirror-redhat-operators` (from generate-cluster-resources.sh)
- Certified catalog: `cs-certified-operator-index-v4-21` (from oc-mirror) or `mirror-certified-operators` (from generate-cluster-resources.sh)
...
> **v3.5 note:** RHOAI 3.5 uses **OGX** (not LlamaStack) and includes the **AI Gateway** (`aigateway`) and **MCP Lifecycle Operator** (`mcplifecycleoperator`) components. These are already configured in the disconnected platform profile.
...
```bash
git add -A
git commit -m "Switch to disconnected profile for RHOAI 3.5"
git push
```
...
Files in this repository relevant to disconnected deployment of RHOAI 3.5:
...
## Version-Specific Notes for RHOAI 3.5

- **AI Gateway (OGX):** RHOAI 3.5 uses the **OGX** framework (replacing LlamaStack from v3.4). The `ogx` and `aigateway` components are enabled in the platform profile.
- **MCP Lifecycle Operator:** New in v3.5, the `mcplifecycleoperator` manages Model Context Protocol servers.
- **Channel:** Use `stable-3.5` for the RHOAI operator channel.
- **Kueue:** Use `stable-v1.3` for the Kueue operator channel.
- **OCP requirement:** Minimum OpenShift version is **4.21**.
