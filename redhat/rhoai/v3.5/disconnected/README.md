...
```bash
cd redhat/rhoai/v3.5/setup/bootstrap
...
```bash
cat redhat/rhoai/v3.5/disconnected/imageset-config-template.yaml
```
...
skopeo copy --all \
  docker://registry.redhat.io/redhat/redhat-operator-index:v4.21 \
...
skopeo copy --all \
  ...
  docker://<INTERNAL_REGISTRY_URL>/redhat/redhat-operator-index:v4.21
...
spec:
  sourceType: grpc
  image: <INTERNAL_REGISTRY_URL>/redhat/redhat-operator-index:v4.21
...
spec:
  sourceType: grpc
  image: <INTERNAL_REGISTRY_URL>/redhat/certified-operator-index:v4.21
...
- Red Hat catalog: `cs-redhat-operator-index-v4-21`
- Certified catalog: `cs-certified-operator-index-v4-21`
